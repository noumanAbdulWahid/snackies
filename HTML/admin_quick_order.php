<?php
    ...
    if(isset($_POST['update_quick_order'])){
        ...
        mysqli_query($conn, "UPDATE `quick_order` SET `quick_order_status` = '$update_payment' WHERE id = '$order_update_id'") or die('query failed');
        $message_update = 'Order status has been updated!';
     
     }
     
     if(isset($_GET['delete'])){
        $delete_id = $_GET['delete'];
        mysqli_query($conn, "DELETE FROM `quick_order` WHERE id = '$delete_id'") or die('query failed');
        header('location:admin_quick_order.php');
     }
?>
<head>
 ...
 <html>
     <body>
         <header>
             ...
             <div class="account-box">
                ...
                <nav class="nav-bar">
                 ...
                 <a class="active" href="../HTML/admin_quick_order.php">Quick Order</a>
         ...
         <section class="orders">
            ...
            <h1 class="title">Quick orders</h1>

            <div class="box-container">
                <?php
                $select_orders = mysqli_query($conn, "SELECT * FROM `quick_order`") or die('query failed');
                ...
                <div class="box">
                    ...
                    <p> order : <span><?php echo $fetch_orders['order']; ?></span> </p>
                    <p> Extra with order : <span><?php echo $fetch_orders['addition_with_order']; ?></span> </p>
                    <p> date and time : <span><?php echo $fetch_orders['date_time']; ?></span> </p>
                    <p> address : <span><?php echo $fetch_orders['address']; ?></span> </p>
                    <p> suggestion : <span><?php echo $fetch_orders['suggestion']; ?></span> </p>
                    <form action="" method="post">
                        <input type="hidden" name="order_id" value="<?php echo $fetch_orders['id']; ?>">
                        <select name="update_quick_order">
                        <option value="" selected disabled><?php echo $fetch_orders['quick_order_status']; ?></option>
                        <option value="pending">pending</option>
                        <option value="completed">completed</option>
                        </select>
                        <input type="submit" value="update" name="update_order" class="option-btn">
                        <a href="admin_quick_order.php?delete=<?php echo $fetch_orders['id']; ?>" onclick="return confirm('delete this order?');" class="btn">delete</a>
                        <?php
                ...
                <?php
                    ...
                    echo '<p class="empty">no quick orders placed yet!</p>';
                }
                ?>
            </div>
